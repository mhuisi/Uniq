...
\begin{figure}[H]
		...
		\end{mathpar}
	\caption{Original linear type system from \citep{wadler_linear_1990} with separate linear and non-linear terms and types. $1$ denotes linear terms, $!$ denotes non-linear terms. ``nonlinear'' demands that the full type or context consists only of non-linear $\to$-types.}
...
\begin{figure}[H]
	...
	\end{mathpar}
	\begin{mathpar}
		$\inferrule[$\mathbb{1}$-Elim]{\Gamma_1 \vdash M :_\sigma \mathbb{1} \\ 0\Gamma_1, x :_0 \mathbb{1} \vdash S :_0 \mathcal{U} \\ \Gamma_2 \vdash N :_\sigma S[()/x]}{\Gamma_1 + \Gamma_2 \vdash \mathrm{let}_\mathbb{1}\ x@() = M\ \mathrm{in}_S\ N :_\sigma S[M/x]}$
	\end{mathpar}

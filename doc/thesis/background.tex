...
\vspace*{12em} % sins have been committed
\begin{mdframed}
\begin{figure}[H]
	\begin{mathpar}
...
\begin{figure}[H]
	...
	\begin{mathpar}
		...
		$\inferrule[$\mathbb{1}$-Intro]{0\Gamma \vdash}{0\Gamma \vdash () :_\sigma \mathbb{1}}$
	\end{mathpar}
	\begin{mathpar}
...
Inspired by Rust \citep{weiss_oxide_2021}, \cite{spiwack_linearly_2022} suggest introducing primitive ``lending'' functions that grant temporary read access into a type, for example as follows for a linear type \icode{MArray} that is allowed to contain other linear types:\\
\begin{code}
	lend : MArray α ⊸ !ℕ ⊸ (!α ⊸ β) ⊸ Array α ⊗ β
\end{code}

Here, the unique reference to the array is consumed by \icode{lend} and hence unavailable in the continuation of type \icode{!α ⊸ β}. \icode{!α} provides read-only access to the array value indexed by the argument of type \icode{!ℕ}. The constraint mechanism additionally ensures that the argument of type \icode{!α} cannot escape in \icode{!α}, as well as that the explicit threading of \icode{Array α} becomes implicit.

For \icode{MArray}, \cite{spiwack_linearly_2022} define another complex borrowing mechanism inspired by Rust: A function \icode{split} allows slicing an array \icode{a} at an index \icode{i}, consuming the original array in the process and producing two linear slices \icode{x := a[0:i)} and \icode{y := a[i:len(a))}, plus a token which witnesses that \icode{x} and \icode{y} are slices of \icode{a}. Then, a function \icode{join} can efficiently put two such slices with an associated witness token back together.
